...
**Transparent offload shim for expensive `cargo` commands.** A gantry crane moves cargo containers between ship and shore; `gantry` moves `cargo` workloads between your machine and a remote executor — without the caller (human or AI agent) having to know it exists.
...
Extraction of a battle-tested in-house pair of bash scripts (`~/.local/bin/cargo` + `cargo-remote`) into a standalone, configurable, publishable tool — in the packaging spirit of [dcg](https://github.com/Dicklesworthstone/destructive_command_guard): single static binary, curl-pipe installer, `doctor` command, transparent to agents.
